...
//  Copyright (c) 2015 Mercury Energy. All rights reserved.
//
...
- (NSMutableDictionary *)dictionaryToSecItemFormat:(NSDictionary *)dictionaryToConvert {
    ...
    if ([[dictionaryToConvert objectForKey:(__bridge id)kSecValueData] isKindOfClass:[NSString class]]) {
        ...
        [returnDictionary setObject:[dictionaryToConvert objectForKey:(__bridge id)kSecValueData] forKey:(__bridge id)kSecValueData];
    }
...
- (NSMutableDictionary *)secItemFormatToDictionary:(NSDictionary *)dictionaryToConvert {
    ...
    if (SecItemCopyMatching((__bridge CFDictionaryRef)returnDictionary, (CFTypeRef *)&passwordData) == noErr) {
        ...
        if (password) {
            ...
            [returnDictionary setObject:(__bridge NSData *)passwordData forKey:(__bridge id)kSecValueData];
        }
    }
    else {
